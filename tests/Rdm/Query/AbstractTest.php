...
/**
 * @covers Rdm_Query_Abstract
 * @runTestsInSeparateProcesses enabled	
 * @preserveGlobalState disabled
 */
class Rdm_QueryTest extends PHPUnit_Framework_TestCase
{
	// ------------------------------------------------------------------------
	
	public function testEmpty()
	{
		$q = new Rdm_Query_Abstract($this->getMock('Rdm_Adapter'));
	...
	{
		$q = new Rdm_Query_Abstract(new stdClass, $p = new stdClass());
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers', 'escape'));
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers'));
		$sub = $this->getMock('Rdm_Query_Select', array('__toString'));
		$sub2 = $this->getMock('Rdm_Query_Select', array('__toString'));
	...
	{
		$q = new Rdm_Query_Abstract(new stdClass);
		
		$this->assertThat($q2 = $q->where(), $this->logicalNot($this->identicalTo($q)));
		$this->assertTrue($q2 instanceof Rdm_Query_Abstract);
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('replaceBinds'));
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers', 'escape'));
		$q_mock = $this->getMock('Rdm_Query_Select', array('__toString'));
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers', 'escapeStr'));
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers', 'getRandomKeyword'));
	...
	{
		$q = $this->getMock('Rdm_Query_Abstract', array('whereIsNull'), array(new stdClass));
