...
require_once dirname(__FILE__).'/../../../lib/Rdm/Query/Select.php';

/**
 * @covers Rdm_Query_Select
 * @runTestsInSeparateProcesses enabled
 * @preserveGlobalState disabled
 */
class Rdm_Query_SelectTest extends PHPUnit_Framework_TestCase
{
	// ------------------------------------------------------------------------
	...
	{
		$q = new Rdm_Query_Select();
	}
	/**
	 * @expectedException Rdm_Query_BuilderException
	 */
	public function testEmpty2()
	{
		$q = new Rdm_Query_Select(new stdClass());
	...
	{
		$q = new Rdm_Query_Select(new stdClass(), $i = new stdClass());
	...
	{
		$q = $this->getMock('Rdm_Query_Select', array('__toString'), array(new stdClass()));
	...
	public function testGetCallsToStringAndPassesToRdm()
	{
		$db = $this->getMock('Rdm_Adapter', array('query'));
		
		$db->expects($this->once())->method('query')->with('this is a test')->will($this->returnValue('this should be a test'));
		
		$q = $this->getMock('Rdm_Query_Select', array('__toString'), array($db));
	...
	{
		$db = $this->getMock('Rdm_Adapter', array('protectIdentifiers'));
	...
	{
		$sq = $this->getMock('Rdm_Query_Select', array('__toString'), array(new stdClass));
	...
	public function testFrom()
	{
		...
		$q = $this->getMock('Rdm_Query_Select', array('column'), array($db));
	...
	public function testFrom3()
	{
		...
		$sq1 = $this->getMock('Rdm_Query_Select', array('__toString'), array(new stdClass));
		$sq2 = $this->getMock('Rdm_Query_Select', array('__toString'), array(new stdClass));
	...
	public function testJoin()
	{
		...
		$q = $this->getMock('Rdm_Query_Select', array('createCondition', 'column'), array($db));
	...
	{
		$db = $this->getMock('Rdm_Adapter', array('escape'));
	...
	/**
	 * Dummy method for the Rdm_Query::createCondition() method.
	 * 
	 * @param  mixed
	 * @param  mixed
	 * @param  array
