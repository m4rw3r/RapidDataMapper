...
require_once dirname(__FILE__).'/../../../lib/Rdm/Query/Update.php';
require_once dirname(__FILE__).'/../../../lib/Rdm/Exception.php';
require_once dirname(__FILE__).'/../../../lib/Rdm/Query/BuilderException.php';

/**
 * @covers Rdm_Query_Update
 * @runTestsInSeparateProcesses enabled
 * @preserveGlobalState disabled
 */
class Rdm_Query_UpdateTest extends PHPUnit_Framework_TestCase
{
	// ------------------------------------------------------------------------
	...
	{
		$q = new Rdm_Query_Update(new stdClass, 'table');
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers', 'prefix', 'escape'));
		...
		$q = new Rdm_Query_Update($mock, 'table');
...
SET "a" = b, "c" = d', $q->getSQL());
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers', 'prefix', 'escape'));
		$q_mock = $this->getMock('Rdm_Query_Select', array('limit', '__toString'));
...
SET "a" = (subquery_mock)', $q->getSQL());
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers', 'prefix', 'escape', '_limit'));
...
WHERE "c" = d_result', $q->getSQL());
	...
	{
		$mock = $this->getMock('Rdm_Adapter', array('protectIdentifiers', 'prefix', 'escape', 'query'));
...
SET "a" = b_result, "c" = d_result'))->will($this->returnValue('db_result'));
	...
	public function testMultipleTables()
	{
		...
		$q = new Rdm_Query_Update($mock, array('table', 'table2'));
