...
class Rdm_Builder_Relation_Construct extends Rdm_Util_Code_MethodBuilder
{
	public function __construct(Rdm_Descriptor_Relation $rel, Rdm_Descriptor $desc)
	{
		...
		$this->setParamList('$object = null, $alias = \'\', '.($desc->isNamespaced() ? '\\' : '').'Rdm_Adapter $db = null');
...
@internal
@param  '.$rel->getRelatedDescriptor()->getClass(true, true).'   The instance to filter by, if the entities to fetch
        '.str_repeat(' ', strlen($rel->getRelatedDescriptor()->getClass(true, true))).'   should relate to this specific instance, optional.
